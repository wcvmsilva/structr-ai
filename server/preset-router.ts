...
export const presetRouter = router({
  ...
  delete: tenantProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const before = await getBundleInTenant(ctx.tenantId, input.bundleId);
      if (!before) {
        throw new TRPCError({ code: "NOT_FOUND", message: `Bundle ${input.bundleId} not found` });
      }
      await deleteBundle(ctx.tenantId, input.bundleId);
      // Match bundle.delete's post-write, best-effort audit contract.
      void logAudit({
        userId: ctx.user.id,
        ...
        console.error("[Audit] Failed to log preset deletion:", error);
      });
