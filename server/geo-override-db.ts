...
/** Verified insert on the caller's transaction; no acquisition, commit or audit. */
async function createOverrideRuleInTx(
  tx: OverrideTx,
  owner: string,
  clean: OverrideRuleCreateData,
...
export async function createOverrideRule(
  ...
  const created = await db.transaction(tx => createOverrideRuleInTx(tx, owner, clean));
...
export type OverrideSeedResult =
  | { seeded: false; existingCount: number }
  | { seeded: true; inserted: number };

/**
 * One business transaction owns discovery, every insert and verified readback.
 * Any own rule (including inactive/manual) preserves the existing skip behavior.
 * This is not a concurrency lock or a new seed/idempotence policy.
 */
export async function seedOverrideRulesForTenant(
  tenantId: string,
  rules: readonly OverrideRuleCreateData[],
  operatorId: string,
): Promise<OverrideSeedResult> {
  const owner = requireOverrideTenant(tenantId, "seedOverrideRulesForTenant");
  let db: OverrideDb;
  try {
    const acquired = await getDb();
    if (!acquired) throw new Error("Database unavailable");
    db = acquired;
  } catch {
    throw new Error("Geographic override rules are unavailable");
  }
  ...
  const result = await db.transaction(async tx => {
    // Scope the catch to discovery: write/readback/commit failures must propagate.
    let existing: GeographicOverride[];
    try {
      existing = await tx.select().from(geographicOverrides)
        .where(eq(geographicOverrides.tenantId, owner));
    } catch {
      throw new Error("Geographic override rules are unavailable");
    }
    if (existing.length > 0) return { seeded: false as const, existingCount: existing.length };

    const created: GeographicOverride[] = [];
    for (const rule of rules) {
      created.push(await createOverrideRuleInTx(tx, owner, pickDefined(rule, CREATE_FIELDS)));
    }
    return { seeded: true as const, created };
  });

  if (!result.seeded) return result;
  // Audit is separately persisted AFTER the whole business batch commits. Null is
  // tolerated; an unexpected throw rejects after all business rows have committed.
  ...
  return { seeded: true, inserted: result.created.length };
}
