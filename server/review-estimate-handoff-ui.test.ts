...
vi.mock('@/lib/trpc',()=>({trpc:{project:{list:{useQuery:()=>({data:{items:[]}})},getById:{useQuery:()=>({data:{id:PROJECT,projectType:'remodel'},isSuccess:true})}},scope:{listDrafts:{useQuery:()=>({data:[]})}},scopeReview:{getReviewState:{useQuery:mocks.review},startReview:{useMutation:()=>({mutate:vi.fn()})},approveOrReject:{useMutation:()=>({mutate:vi.fn()})},convertToBundle:{useMutation:()=>({mutate:vi.fn()})}},scopeCompleteness:{getChecklist:{useQuery:()=>({data:{projectType:'remodel',items:[],summary:''},isSuccess:true})}},estimate:{createFromScopeDraft:{useMutation:mocks.estimate}},useUtils:()=>({estimate:{list:{invalidate:mocks.invalidate}}})}}));
