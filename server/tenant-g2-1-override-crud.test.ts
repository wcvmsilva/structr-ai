...
beforeEach(() => {
  ...
  boundary.seed.mockResolvedValue({ seeded: true, inserted: COASTAL_OVERRIDE_SEED_RULES.length });
...
describe("G2-1 normal caller context propagation", () => {
  ...
  // Boundary proof only. Real batch discovery/transactions have their own suite.
  it("seedCoastalRules delegates the mapped batch with the trusted tenant and actor", async () => {
    ...
    expect(boundary.seed).toHaveBeenCalledWith(TENANT_A, COASTAL_OVERRIDE_SEED_RULES.map(rule => ({
      zone: rule.zone, trade: rule.trade, finishLevel: rule.finishLevel,
      originalAssemblyId: rule.originalAssemblyId, replacementAssemblyId: rule.replacementAssemblyId,
      overrideType: rule.overrideType, reasonTemplate: rule.reasonTemplate, isActive: rule.active,
    })), ACTOR);
    expect(boundary.list).not.toHaveBeenCalled();
    expect(boundary.create).not.toHaveBeenCalled();
  });
