...
// Both the root database and its transaction expose this read interface.
type BundleReadHandle = Pick<PostgresJsDatabase, "select">;
...
export async function addItemToBundle(tenantId: string, data: {
  ...
  return db.transaction(async (tx) => {
    ...
    const parent = await loadBundleInTenant(tx, tenantId, data.bundleId);
    ...
    const [result] = await tx.insert(bundleItems).values({
      bundleId: parent.id,
...
export async function updateBundleItemQuantity(
  ...
  return db.transaction(async (tx) => {
    const existing = await loadBundleItemInTenant(tx, tenantId, bundleItemId);
    ...
    await tx.update(bundleItems).set({
      quantity,
    }).where(and(eq(bundleItems.id, existing.id), eq(bundleItems.bundleId, existing.bundleId)));

    const [updated] = await tx.select().from(bundleItems).where(eq(bundleItems.id, existing.id)).limit(1);
    return updated;
  });
}
