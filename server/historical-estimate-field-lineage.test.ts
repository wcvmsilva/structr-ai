// C2-A materialization is held before lineage reads; budget-read lineage assertions remain.
...
describe.each(['source','link'] as const)('historical field ancestry detected by %s',kind=>{
  ...
  it('blocks direct materialization and replay before any field event or budget write',async()=>{setup(kind);for(let retry=0;retry<2;retry++)await expect(materializeChangeOrderTasks({changeOrderId:CO,userId:USER})).rejects.toMatchObject({code:'LEGACY_ESTIMATE_OPERATION_UNAVAILABLE'});expect(writes).toEqual([]);});
  it('detects a historical ancestor behind a persisted calculated revision',async()=>{setup(kind,true);expect(await listApprovedChangeOrders(PROJECT)).toEqual([]);await expect(materializeChangeOrderTasks({changeOrderId:CO,userId:USER})).rejects.toMatchObject({code:'LEGACY_ESTIMATE_OPERATION_UNAVAILABLE'});expect(writes).toEqual([]);});
});
...
it('fails closed on a cycle without recursive looping or writes',async()=>{state.estimate_drafts.push(draft({id:CO,changeOrderOf:VERSION}),draft({id:VERSION,supersedesId:CO}));await expect(materializeChangeOrderTasks({changeOrderId:CO,userId:USER})).rejects.toMatchObject({code:'LEGACY_ESTIMATE_OPERATION_UNAVAILABLE'});expect(await listApprovedChangeOrders(PROJECT)).toEqual([]);expect(reads).toBeLessThan(20);expect(writes).toEqual([]);});
it('fails closed when an ancestor is missing',async()=>{state.estimate_drafts.push(draft({id:CO,changeOrderOf:id(88)}));await expect(materializeChangeOrderTasks({changeOrderId:CO,userId:USER})).rejects.toMatchObject({code:'LEGACY_ESTIMATE_OPERATION_UNAVAILABLE'});expect(writes).toEqual([]);});
