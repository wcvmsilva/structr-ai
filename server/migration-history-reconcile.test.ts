...
describe("migration history command contract", () => {
  ...
  it("emits the six expected identities and exit 2 when no environment evidence was supplied", () => {
    const { log, error } = output();
    expect(runMigrationHistoryCli([])).toBe(2);
    const report = JSON.parse(log.mock.calls[0][0]);
    expect(report.local.migrations).toHaveLength(6);
  ...
  it("returns exit 1 for a valid snapshot whose observed ledger is empty", () => {
    ...
    expect(JSON.parse(log.mock.calls[0][0]).drizzle.missingLocalTags).toHaveLength(6);
  });
...
describe("offline ledger comparison", () => {
  ...
  it("reconciles the actual repository's six-file journal as a bounded identity inventory", () => {
    ...
    expect(reconcileMigrationHistory(local, { ...snapshot(), drizzle: { available: true, rows: [] }, supabase: { available: true, rows: [] } }).drizzle.missingLocalTags).toHaveLength(6);
  });
});
