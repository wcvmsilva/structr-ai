...
export type G2PointPostgres = {
  directory: string;
  observer: G2PointConnection;
  connect: (name: string) => Promise<G2PointConnection>;
  stop: () => Promise<void>;
};
 ...
 */
export async function startG2PointPostgres(factory: typeof postgres): Promise<G2PointPostgres> {
  if (process.env.G2_1_POSTGRES !== "1") throw new Error("G2Point requires G2_1_POSTGRES=1");
  const inherited = Object.keys(process.env).filter(key =>
    /^PG/i.test(key) || /^(DATABASE_URL|POSTGRES_URL|POSTGRESQL_URL)$/i.test(key),
  );
  if (inherited.length) {
    throw new Error(`G2Point refuses inherited database configuration: ${inherited.join(", ")}`);
  }
  for (const executable of ["initdb", "pg_ctl", "postgres"]) {
    await access(join(binDirectory, executable), constants.X_OK);
  }

  const directory = await mkdtemp("/private/tmp/structr-g2point-pg-");
  ...
  const assertOwnedPidFile = () => {
    ...
    if (ownedData !== data || !/^\d+$/.test(pid)) {
      throw new Error(`G2Point cannot verify owned server; retained directory: ${directory}`);
    }
  };
  ...
  const emergencyCleanup = () => {
    ...
    if (existsSync(pidFile)) {
      ...
      if (result.status !== 0 && existsSync(pidFile)) {
        ...
        if (existsSync(pidFile)) {
          throw new Error(`G2Point could not stop its owned server; retained directory: ${directory}`);
        }
      }
    }
  ...
  const signalCleanup = (exitCode: number) => {
    void stop().then(() => process.exit(exitCode), error => {
      console.error("G2Point PostgreSQL shutdown failed", error);
      process.exit(1);
    });
  };
  ...
  try {
    ...
    await runFile(join(binDirectory, "initdb"),
      ["-D", data, "-U", "g2point_runner", "--auth-local=trust", "--auth-host=reject", "--encoding=UTF8", "--locale=C"],
    ...
    const connect = async (name: string): Promise<G2PointConnection> => {
      const queries: G2PointConnection["queries"] = [];
      const options: postgres.Options<{}> = {
        ...
        connection: { application_name: `g2point-${name}`, TimeZone: "UTC", statement_timeout: 5000, lock_timeout: 4000 },
        onnotice: () => undefined,
      ...
      if (identity.host !== null || identity.database !== "postgres" || identity.role !== "g2point_runner") {
        throw new Error("G2Point connection did not reach its owned Unix-socket cluster");
      }
    ...
    // Exact geographic override columns, defaults, primary key and NOT NULL constraints.
    // External FKs and RLS are omitted: these fixtures prove CRUD rule ownership only,
    // not catalog integrity, production policy, migrations or operational seed behavior.
    ...
    try { await stop(); } catch (cleanupError) {
      throw new AggregateError([error, cleanupError], "G2Point PostgreSQL startup and cleanup failed");
    }
    throw new Error(`G2Point PostgreSQL setup failed${detail ? `:\n${detail}` : ""}`, { cause: error });
  }
}
