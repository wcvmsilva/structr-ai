...
const port = 55443; // Socket filename only: the server has listen_addresses=''.
...
export type AppPrincipalProvenance = Readonly<{
  directory: string;
  socket: string;
  nonce: string;
}>;
const ownedConnections = new WeakMap<
  ...
  AppPrincipalProvenance
>();

export function assertOwnedAppPrincipalConnection(
  sql: ReturnType<typeof postgres>
): AppPrincipalProvenance {
  const owned = ownedConnections.get(sql);
  if (!owned) {
    throw new Error(
      "App-principal lab refuses a client not created by its owned harness"
    );
  }
  if (
    sql.options.host.length !== 1 ||
    sql.options.host[0] !== owned.socket ||
    sql.options.port.length !== 1 ||
    sql.options.port[0] !== port ||
    sql.options.database !== "postgres" ||
    sql.options.ssl !== false
  ) {
    throw new Error(
      "App-principal lab connection options no longer match its owned cluster"
    );
  }
  return owned;
}

export type AppPrincipalConnection = {
  sql: ReturnType<typeof postgres>;
  db: PostgresJsDatabase;
  pid: number;
};

export type AppPrincipalCluster = {
  directory: string;
 ...
 */
export async function startAppPrincipalPostgres(
  factory: typeof postgres
): Promise<AppPrincipalCluster> {
  if (process.env.APP_PRINCIPAL_LAB !== "1")
    throw new Error("App-principal lab requires APP_PRINCIPAL_LAB=1");
  const inherited = Object.keys(process.env).filter(
    key =>
      /^PG/i.test(key) ||
      /^(?:(?:.*_)?(?:DATABASE|POSTGRES|POSTGRESQL|DB)_URL(?:_.*)?|SUPABASE_DB_URL)$/i.test(
        key
      )
  );
  if (inherited.length) {
    throw new Error(
      `App-principal lab refuses inherited database configuration: ${inherited.join(", ")}`
    );
  }
  ...
  const directory = await mkdtemp("/private/tmp/structr-app-principal-pg-");
  ...
  const assertOwnedPidFile = () => {
    ...
    if (ownedData !== data || !/^\d+$/.test(pid)) {
      throw new Error(
        `App-principal lab cannot verify owned server; retained directory: ${directory}`
      );
    }
  };
  ...
  const emergencyCleanup = () => {
    ...
    if (existsSync(pidFile)) {
      ...
      if (result.status !== 0 && existsSync(pidFile)) {
        ...
        if (existsSync(pidFile)) {
          throw new Error(
            `App-principal lab could not stop its owned server; retained directory: ${directory}`
          );
        }
      }
    }
  ...
  const signalCleanup = (exitCode: number) => {
    void stop().then(
      ...
      error => {
        console.error("App-principal lab PostgreSQL shutdown failed", error);
        process.exit(1);
      }
    );
  };
  ...
  try {
    ...
    await runFile(
      ...
      [
        ...
        log,
        "-o",
        `-k ${socket} -h '' -p ${port} -c unix_socket_permissions=0700 -c app_principal_lab.nonce=${nonce} -c statement_timeout=5000 -c idle_in_transaction_session_timeout=10000`,
        "-w",
        "-t",
        "15",
        "start",
    ...
    const connectOwned = async (
      ...
      const options: postgres.Options<{}> = {
        ...
        connection: {
          application_name: `app-principal-${name}`,
          TimeZone: "UTC",
      ...
      >`
        SELECT pg_backend_pid() AS pid, inet_server_addr()::text AS host,
          ...
          current_setting('app_principal_lab.nonce', true) AS nonce,
      ...
      if (
        ...
        identity.role !== role ||
        identity.session_role !== role ||
        identity.nonce !== nonce ||
        client.options.host.length !== 1 ||
        client.options.host[0] !== socket ||
        client.options.port.length !== 1 ||
        client.options.port[0] !== port ||
        client.options.database !== "postgres" ||
        client.options.user !== role ||
        client.options.ssl !== false ||
        identity.port !== String(port) ||
        identity.listeners !== ""
      ) {
        await client.end({ timeout: 1 });
        throw new Error(
          "App-principal lab connection did not reach its owned Unix-socket cluster and role"
        );
      }
      if (role !== "app_principal_runner")
        ownedConnections.set(client, provenance);
      return { sql: client, db: drizzle(client), pid: identity.pid };
    };
    const bootstrap = await connectOwned("bootstrap", "app_principal_runner");
    const [owned] = await bootstrap.sql<
      { directory: string; sockets: string }[]
    >`
    ...
    if (owned.directory !== data || owned.sockets !== socket) {
      throw new Error("App-principal lab data/socket directory mismatch");
    }
    // Roles and grants below exist only inside this newly owned cluster. No membership
    // grants and no switchable privileged account are available to either app login.
    await bootstrap.sql.unsafe(`
      ...
      CREATE ROLE app_denied LOGIN NOINHERIT NOSUPERUSER NOCREATEDB NOCREATEROLE NOBYPASSRLS;
      GRANT USAGE ON SCHEMA public TO app_runtime, app_denied;
    `);
    const connect = (
      name: string,
    ...
    try {
      ...
      throw new AggregateError(
        [error, cleanupError],
        "App-principal lab PostgreSQL startup and cleanup failed"
      );
    }
    throw new Error(
      `App-principal lab PostgreSQL setup failed${detail ? `:\n${detail}` : ""}`,
      { cause: error }
    );
  }
}
