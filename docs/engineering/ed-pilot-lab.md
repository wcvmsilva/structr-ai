...
- The child receives an explicit environment allowlist, an ephemeral session key, dotenv disabled and empty Vite environment/private cache directories. `HOME` is not reassigned. No existing database URL or application secret is accepted.
...
The readiness maintenance separately corrects discount presentation, displays the real policy result and disables download attempts when authorization is unresolved or denied. CSV keeps its existing authorization/validation/reconciliation chain. Legacy PDF/JSON endpoints still enforce read access without the same lifecycle gate; UI disabling is not equivalent backend enforcement.

This laboratory does not implement partial commercial acceptance, a CSV importer, a second operator/approver or full Lead-to-Export continuity. Project reuse in Intake, mounted review-to-draft continuity and end-to-end source-code lineage remain separate integration work. Tax treatment, payment terms and business classification cannot be inferred from the fixture.
