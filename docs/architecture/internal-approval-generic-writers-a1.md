...
The new suites contain 213 behavior cases: 118 mutation cases, 31 creation cases and 64 route cases. All passed locally after recorded RED failures. The focused compatibility runs additionally passed 284 existing cases (66 historical guards, 51 Phase2 flow, 16 status guards, 46 tenant/bundle controls, four priced persistence, seven component-continuity, 20 geo caller and 74 document-export authorization cases). These are unit/integration fakes, not new physical database proofs. Exact-commit GitHub CI is recorded in the pull request; this document alone is not evidence that the full release gates passed.

The first complete CI run exposed 19 compatibility failures: the new static authorization import entered a partial-database-mock initialization cycle in the unchanged geo caller suite, and the export authorization fixture lacked the new transaction/context API. Loading authorization inside generic creation's transaction resolved the former with all 20 geo caller assertions unchanged. The export fixture now models the transaction and precise identity bindings, preserving all 74 original controls and adding no-write/audit/payload checks on the H1 refusals. No authorization rule or timeout was relaxed.
