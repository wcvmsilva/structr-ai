...
| Effective principal and privileges | At the original observation, scoped lead/pipeline reads and a diagnostic path switched to `postgres`. The [bounded follow-up](effective-app-principal-2026-09-19.md) removes those switches and proves activity/audit rollback using an owned unprivileged PostgreSQL principal. Legacy writers still require `authenticated` and a verified subject/profile contract; the negative tests deliberately expose that remaining incompatibility. A development connection or laboratory result does not establish the deployed principal. |
